...
ADRs are immutable once `Status: Accepted`. To change a decision, write a new ADR that supersedes it and update the prior ADR's `Status` line to `Superseded by ADR-NNNN`. Do not edit the body of an accepted ADR to reflect later changes — factual drift (paths renamed, tools upgraded, references that have moved) is expected and not corrected in the ADR. Current-state truth lives in this file, runbooks, and the code itself; ADRs are dated snapshots of why a decision was made at the time.
